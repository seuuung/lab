...
function verify(eye, center, cubePos, fov, w, h) {
    const aspect = w / h;
    ...
    const topW = pTR.x - pTL.x;
    const botW = pBR.x - pBL.x;
    const leftH = pBL.y - pTL.y;
    const rightH = pBR.y - pTR.y;
    
    const sCube = project(cubePos, eye, center, fov, aspect, w, h);
    const sShadow = project(shadowCenter, eye, center, fov, aspect, w, h);
    
    console.log(`[Screen ${w}x${h}] Eye=[${eye.join(',')}]`);
    console.log(`  Top Width: ${topW.toFixed(2)}px, Bot Width: ${botW.toFixed(2)}px (Diff: ${(topW-botW).toFixed(4)}px)`);
    console.log(`  Left Height: ${leftH.toFixed(2)}px, Right Height: ${rightH.toFixed(2)}px (Diff: ${(leftH-rightH).toFixed(4)}px)`);
...
console.log('=== VERIFYING ZERO DISTORTION CAMERA SETUP ===');
verify([0, 0.8, 27], [0, -0.5, -4], [0, -2.8, 0], 50, 390, 844);
verify([0, 0.5, 25], [0, -0.5, -3], [0, -2.2, 0], 44, 1200, 800);
